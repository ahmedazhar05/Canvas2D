...
function draw() {
  ...
  //displays all pencil shapes
  if(typeof(shapes["Pencil"])!='undefined'){
    for(ob of shapes["Pencil"]){
      beginShape();
      for(let i=0;i<ob.length;i+=2)
        vertex(ob[i], ob[i+1]);
      endShape();
    }
  }
  ...
  //when save button from export dialog box is clicked
    ...
    if(typeof(shapes["Pencil"])!='undefined'){
      for(ob of shapes["Pencil"]){
        ...
        for(let i=0;i<ob.length;i+=2)
          content.push("vertex("+ob[i]+", "+ob[i+1]+");");
